 ...
 *
 * The three items he named by name are all **read-only** in this drawer, and
 * each one is read-only for a different reason that is worth writing down
 * rather than discovering:
 *
 * 1. **Name.** MAR-589's ruling — taken by Henrik during this issue — is that
 *    the display name is first-class and the id is a value. The reading half of
 *    that ships here. The writing half cannot: the `agents` table has columns
 *    `name`, `manifest_version`, `manifest_json`, `imported_at`, `avatar`, and
 *    no name of DASH's own, so the name on screen is the *author's*
 *    `display_name` off the manifest. A rename needs a migration, a store
 *    write, an ipc command and a preload method.
 ...
 *
 * **Showing them read-only is the point, not a consolation.** A settings drawer
 * that simply omitted the three things somebody asked for reads as a drawer
 * that forgot them. One that shows each with its provenance and where it is
 * changed reads as a product that knows its own edges — the same discipline
 * `ManifestGapNotice` applies when DASH is about to do less than it could.
 *
      ...
      <section className="agent-settings-block">
        ...
        <dl className="facts">
          <div>
            ...
            <dd>{title}</dd>
            <dd className="muted">{AGENT_SETTINGS_COPY.identity.name_source}</dd>
          </div>
          <div>
