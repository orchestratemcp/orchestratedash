...
/**
 * One frame, with the retry the first attempt after a size change needs.
 *
 * `capturePage()` rejects with `UnknownVizError` on the first call after the
 * window has been resized or the page has reflowed under it — the compositor has
 * torn down the old surface and not yet published a new one. It is deterministic
 * rather than flaky: the *second* call, a moment later, succeeds.
 *
 * Without this a scene dies partway through and leaves a directory that looks
 * like a complete run minus a few files, which is the worst of both — a reviewer
 * cannot tell a frame that was never taken from one the harness decided to skip.
 * This run met it at 768/compact, nine images in.
 */
async function shoot(target: BrowserWindow, name: string): Promise<void> {
  ...
  try {
    ...
    console.log(`[deploy] ${name}: first capture failed (${String(error)}) — retrying once`);
    await settle(800);
