/*
 * The Agent Kit template as it stood before MAR-887 split the runtime out of
 * it — one monolithic file with no import, taken verbatim from
 * `agent-kit/template/agent.mjs` at commit cb2cb1c.
 *
 * It is here to be spawned, not read. Every agent a person generated before
 * the SDK existed is a copy of these bytes sitting in their own folder, and
 * nothing in DASH rewrites it: the runner spawns whatever `agent.mjs` a
 * registration names. `tests/legacy-agent.test.ts` therefore runs this file
 * under a real `Supervisor` and holds it to the same telemetry, artifact and
 * acknowledgement behaviour the current template is held to. If that test goes
 * red, agents that exist on people's disks today have stopped working.
 *
 * Do not update this file to match the template. Its whole value is that it
 * does not.
 */
