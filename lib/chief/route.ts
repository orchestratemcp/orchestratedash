/**
 * Who the Chief hands a request to, and why (MAR-419, DASH-15).
 *
 * ## This slice has no model, and that is the design rather than a stub
 *
 * MAR-419's scope names a model (DASH-14) with tools bound to the command
 * channel. Nothing in DASH talks to one yet, so a "Chief" written around a model
 * would be a mock of the interesting half with none of it working. What the
 * issue also states, separately and in its own sentence, is the routing rule:
 *
